...
***** Clojure Environment
- Clojure CLI (deps.edn) + Babashka tasks
...
#+BEGIN_SRC shell
# Optional: Install Emacs packages for org-mode support
...
manifest (it replaced =project.clj=) and =bb.edn= provides the tasks. The
Makefile's =test=, =lint=, =lint-fix= and =run= targets delegate to them.
...
| =bb run=                | print the domain overview (=clojure -M:run=)             |
| =bb repl=               | rebel-readline REPL with Portal (=clojure -M:dev:rebel=) |

The former Leiningen commands map to aliases:

| Leiningen                   | Clojure CLI                                                         |
|-----------------------------+---------------------------------------------------------------------|
| =lein run= / =lein test=    | =bb run= / =bb test=                                                |
| =lein rebel= / =lein portal= | =clojure -M:dev:rebel= / =clojure -M:portal=                       |
| =lein with-profile +lcl=    | =clojure -M:dev:lcl= (and =export endpoint-override=http://localhost:4566=) |
| =lein with-profile +dev-aws= | =clojure -M:dev:dev-aws= (and =export AWS_PROFILE=dev=)            |
| =lein kibit= / =lein eastwood= | =clojure -M:kibit= / =clojure -M:eastwood=                       |

*** Specs

Data specs live in =src/aif_c01/specs.clj=, and each public fn in the domain
namespaces has an =s/fdef= right after its =defn= (see the
[[https://clojure.org/guides/spec][clojure.spec guide]]).
=test/aif_c01/specs_test.clj= runs =stest/check= over every pure fn. The AWS,
network and printing fns in =aif-c01.d0-setup.environment= are instrumented but
not generatively checked. The unit tests run with instrumentation on, and
...
*** AWS Practice Commands
#+BEGIN_SRC shell
# Practice with AWS resources (audit only)
make aws-practice

# Clean up AWS resources used in practice
make aws-practice-cleanup

# AWS Rekognition - Count people in image
make aws-rekognition

# AWS Polly - Create a story
make aws-polly
#+END_SRC

*** Study Resources Generation
#+BEGIN_SRC shell
# Generate study resources (ODT format)
make study-resources

# Tangle code blocks from org files
make tangle

# Generate diagrams from org files
